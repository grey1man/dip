...
async function ask_hours()
{
  let data = {first_name : localStorage.getItem('first_name'),
    ...
    let response = await fetch('http://217.28.239.20:3000/hours', 
    {
      method : 'POST',
      headers: {
...
function new_row(val)
{
  ...
  elem.innerHTML = '<tr> \
  <td > <input onchange="save();" type="text" size="5"> </td> \
  <td>  <input style="width: 350px" size="10" list="themes" onchange="save(); new_row(this)" contenteditable="true"> </td> \
...
function hours_evalv()
{
  ...
  while (table_2.children[i].id != 'total')
  {
    ...
    var test = tmp.children[5].children[0].value
    if (tmp.children[2].children[0].value != "")
    {
      lec += parseInt(tmp.children[2].children[0].value)
    }
    if (tmp.children[3].children[0].value != "")
    {
      prac += parseInt(tmp.children[3].children[0].value)
    }
    if (tmp.children[4].children[0].value != "")
    {
      lab += parseInt(tmp.children[4].children[0].value)
    }
    if (tmp.children[5].children[0].value != "")
    {
...
function acc_change()
{
  window.location.href = "http://217.28.239.20:3000/acc";
}
